...
- **SQLite** (`src/ansina/storage/`, issue #6): the only external dependency in M0. One file on disk, WAL journal mode, path set via `[database] path` / `ANSINA_DATABASE__PATH` (defaults to `./ansina.db`; WAL adds `-wal`/`-shm` sidecars next to it). No network service, no separate process.
